...
console.log('====================================================');
console.log('🚀 [CLI] 초기 DB 일괄 마이그레이션 파이프라인 가동');
...
async function runMigration() {
  ...
  console.log('📦 [1/5] 엑셀 5개 시트 파싱 및 스키마 변환 중...');
  ...
  const customerBillingGroup = new Map();
  ...
  rawBillRows.forEach((r, rowIdx) => {
    ...
    const startDate = sanitizeExcelDate(r[4]) || sanitizeExcelDate(r[3]) || '2026-08-01';
    ...
    contractHistories.push({
      ...
      description: `레거시 엑셀 초기 마이그레이션 적재 (계약구분: ${contractType})`,
      createdAt: nowIso
    });
    ...
    if (ownAssetNo) {
      ...
      matchedAssetId = existingAsset.id;
      ...
      if (!leaseAsset) {
        ...
        leaseAsset = {
          ...
          renter: leaseVendor,
          rentStart: sanitizeExcelDate(r[18]) || startDate,
      ...
      externalLeases.push({
        ...
        monthlyRentFee: sanitizeNumber(r[20]),
        dailyRentFee: Math.round(sanitizeNumber(r[20]) / 30),
        leaseStartDate: sanitizeExcelDate(r[18]) || startDate,
        leaseEndDate: sanitizeExcelDate(r[19]),
        status: r[19] ? 'RETURNED' : 'ACTIVE',
        memo: r[16] ? `협착소유: ${r[16]}` : '',
        createdAt: nowIso,
        updatedAt: nowIso
      });
    ...
    if (matchedAssetId) {
      ...
      assetInOutLogs.push({
        ...
        description: `초기 마이그레이션 출고 등록 (${site.name})`,
        createdAt: nowIso
      });
    }
    ...
    const totalRowBill = sanitizeNumber(r[25]);
    const days = sanitizeNumber(r[6]) || 30;

    excelTotalBillingSum += totalRowBill;

    let custBill = customerBillingGroup.get(customer.id);
    if (!custBill) {
      custBill = {
        customer: customer,
      ...
      customerBillingGroup.set(customer.id, custBill);
    }

    if (monthRentFee > 0 || totalRowBill > 0) {
      custBill.details.push({
        ...
        itemType: 'RENTAL',
        quantity: days,
        unitPrice: isTerminated ? dailyFee : monthRentFee,
        amount: monthRentFee || totalRowBill,
        description: `2026-08 렌탈료 (${days}일 가동, 현장: ${site.name})`,
        internalDescription: `원천행 ${rowIdx + 4}: ${rawCustName} / ${targetModel}`
      });
      custBill.totalAmount += (monthRentFee || totalRowBill);
    }
    ...
    if (transportFee > 0) {
      ...
      custBill.totalAmount += transportFee;
    }
    ...
    if (otherFee > 0) {
      ...
      custBill.totalAmount += otherFee;
    }
  });

  customerBillingGroup.forEach((group, custId) => {
  ...
  console.log(`- 계약 자산 슬롯: ${contractAssets.length} 건`);
  console.log(`- 외부 전대 대장: ${externalLeases.length} 건`);
  console.log(`- 2026-08 청구서: ${billings.length} 건`);
  console.log(`- 청구 상세 품목: ${billingDetails.length} 건`);
  console.log(`- 청구 총액: ₩${billings.reduce((acc, b) => acc + b.totalAmount, 0).toLocaleString()}`);
  console.log(`- 엑셀 원본 청구합: ₩${excelTotalBillingSum.toLocaleString()}`);
...
async function main() {
  try {
    ...
    console.log('\n✅ 엑셀 데이터 파싱 및 무결성 검증 완료!');
    console.log('💡 UI 또는 CLI를 통해 Supabase 적재를 진행할 수 있습니다.');
  } catch (err) {
